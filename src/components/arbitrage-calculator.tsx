'use client';

import { useState, useMemo } from 'react';
import type { CoinPremium, MultiCoinKimpData, ArbitrageResult } from '@/lib/types';
import { calculateArbitrage } from '@/lib/kimp';

interface ArbitrageCalculatorProps {
  data: MultiCoinKimpData;
  selectedCoin?: CoinPremium | null;
}

const PRESET_AMOUNTS = [1_000_000, 5_000_000, 10_000_000, 50_000_000];

export default function ArbitrageCalculator({ data, selectedCoin }: ArbitrageCalculatorProps) {
  const [coinSymbol, setCoinSymbol] = useState(selectedCoin?.symbol || 'BTC');
  const [amountInput, setAmountInput] = useState('10000000');
  const [direction, setDirection] = useState<'buy-kr-sell-global' | 'buy-global-sell-kr'>('buy-global-sell-kr');

  const coin = data.coins.find(c => c.symbol === coinSymbol);
  const amount = parseInt(amountInput.replace(/,/g, ''), 10) || 0;

  // 선택된 코인이 외부에서 바뀌면 반영
  if (selectedCoin && selectedCoin.symbol !== coinSymbol) {
    setCoinSymbol(selectedCoin.symbol);
  }

  const result: ArbitrageResult | null = useMemo(() => {
    if (!coin || amount <= 0) return null;
    return calculateArbitrage(coin, amount, data.usdKrw, direction);
  }, [coin, amount, data.usdKrw, direction]);

  // 자동 추천 방향
  const recommendedDirection = coin
    ? coin.premium > 0 ? 'buy-global-sell-kr' : 'buy-kr-sell-global'
    : 'buy-global-sell-kr';

  return (
    <div className="rounded-2xl bg-gray-900 border border-gray-800 p-6">
      <h2 className="text-sm font-medium text-gray-400 mb-4">차익거래 계산기</h2>

      {/* 입력 폼 */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
        {/* 코인 선택 */}
        <div>
          <label className="text-xs text-gray-500 block mb-1">코인</label>
          <select
            value={coinSymbol}
            onChange={e => setCoinSymbol(e.target.value)}
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-gray-600"
          >
            {data.coins.map(c => (
              <option key={c.symbol} value={c.symbol}>
                {c.symbol} ({c.premium >= 0 ? '+' : ''}{c.premium.toFixed(2)}%)
              </option>
            ))}
          </select>
        </div>

        {/* 투자 금액 */}
        <div>
          <label className="text-xs text-gray-500 block mb-1">투자 금액 (원)</label>
          <input
            type="text"
            value={Number(amountInput).toLocaleString()}
            onChange={e => setAmountInput(e.target.value.replace(/[^0-9]/g, ''))}
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white font-mono focus:outline-none focus:ring-1 focus:ring-gray-600"
          />
          <div className="flex gap-1 mt-1">
            {PRESET_AMOUNTS.map(a => (
              <button
                key={a}
                onClick={() => setAmountInput(String(a))}
                className="text-[10px] px-1.5 py-0.5 rounded bg-gray-800 text-gray-500 hover:text-gray-300 transition"
              >
                {(a / 10000).toFixed(0)}만
              </button>
            ))}
          </div>
        </div>

        {/* 방향 */}
        <div>
          <label className="text-xs text-gray-500 block mb-1">거래 방향</label>
          <div className="flex flex-col gap-1">
            <button
              onClick={() => setDirection('buy-global-sell-kr')}
              className={`text-xs px-3 py-1.5 rounded-lg transition ${
                direction === 'buy-global-sell-kr'
                  ? 'bg-red-900/50 text-red-300 ring-1 ring-red-700'
                  : 'bg-gray-800 text-gray-500 hover:text-gray-300'
              }`}
            >
              해외 매수 → 한국 매도
              {recommendedDirection === 'buy-global-sell-kr' && ' ★'}
            </button>
            <button
              onClick={() => setDirection('buy-kr-sell-global')}
              className={`text-xs px-3 py-1.5 rounded-lg transition ${
                direction === 'buy-kr-sell-global'
                  ? 'bg-blue-900/50 text-blue-300 ring-1 ring-blue-700'
                  : 'bg-gray-800 text-gray-500 hover:text-gray-300'
              }`}
            >
              한국 매수 → 해외 매도
              {recommendedDirection === 'buy-kr-sell-global' && ' ★'}
            </button>
          </div>
        </div>
      </div>

      {/* 결과 */}
      {result && (
        <div className={`rounded-xl p-4 ${result.viable ? 'bg-green-900/20 border border-green-800/50' : 'bg-red-900/20 border border-red-800/50'}`}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                result.viable ? 'bg-green-900/50 text-green-300' : 'bg-red-900/50 text-red-300'
              }`}>
                {result.viable ? '수익 가능' : '손실 예상'}
              </span>
              <span className="text-xs text-gray-500">{result.estimatedTime}</span>
            </div>
            <p className={`text-2xl font-bold font-mono ${result.netProfit >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              {result.netProfit >= 0 ? '+' : ''}{result.netProfit.toLocaleString('ko-KR', { maximumFractionDigits: 0 })}원
            </p>
          </div>

          <div className="text-xs text-gray-500 mb-2">{result.direction}</div>

          {/* 수익률 바 */}
          <div className="w-full h-2 bg-gray-800 rounded-full mb-3 overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${result.netProfit >= 0 ? 'bg-green-500' : 'bg-red-500'}`}
              style={{ width: `${Math.min(Math.abs(result.netProfitRate) * 20, 100)}%` }}
            />
          </div>

          {/* 상세 내역 */}
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
            <div className="flex justify-between">
              <span className="text-gray-500">투자금액</span>
              <span className="text-gray-300 font-mono">{result.investmentKrw.toLocaleString()}원</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">현재 김프</span>
              <span className="text-gray-300 font-mono">{result.premium >= 0 ? '+' : ''}{result.premium.toFixed(2)}%</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">총 수익 (수수료 전)</span>
              <span className="text-gray-300 font-mono">{result.grossProfit.toLocaleString('ko-KR', { maximumFractionDigits: 0 })}원</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">순수익률</span>
              <span className={`font-mono font-medium ${result.netProfitRate >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                {result.netProfitRate >= 0 ? '+' : ''}{result.netProfitRate.toFixed(3)}%
              </span>
            </div>

            <div className="col-span-2 border-t border-gray-800 my-1" />

            <div className="flex justify-between text-gray-600">
              <span>한국 거래소 수수료</span>
              <span className="font-mono">-{result.exchangeFeeKr.toLocaleString('ko-KR', { maximumFractionDigits: 0 })}원</span>
            </div>
            <div className="flex justify-between text-gray-600">
              <span>해외 거래소 수수료</span>
              <span className="font-mono">-{result.exchangeFeeGlobal.toLocaleString('ko-KR', { maximumFractionDigits: 0 })}원</span>
            </div>
            <div className="flex justify-between text-gray-600">
              <span>네트워크 수수료</span>
              <span className="font-mono">-{result.networkFee.toLocaleString('ko-KR', { maximumFractionDigits: 0 })}원</span>
            </div>
            <div className="flex justify-between text-gray-600">
              <span>예상 슬리피지</span>
              <span className="font-mono">-{result.slippage.toLocaleString('ko-KR', { maximumFractionDigits: 0 })}원</span>
            </div>
          </div>
        </div>
      )}

      {!coin && (
        <p className="text-sm text-gray-500 text-center py-4">코인을 선택해주세요.</p>
      )}
    </div>
  );
}
