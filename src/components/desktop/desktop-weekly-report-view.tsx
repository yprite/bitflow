...
function NewsCard({
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex items-center gap-2 text-[10px] font-mono uppercase tracking-[0.16em] text-dot-muted">
        ...
        <DesktopHero
          ...
          sidebar={report ? (
            <div className="space-y-4">
              <DesktopStatCard label="회차 기간" value={`${formatDate(report.weekStart)} - ${formatDate(report.weekEnd)}`} tone="neutral" />
              <DesktopStatCard label="뉴스 후보" value={report.newsItems.length} tone="neutral" />
              <DesktopStatCard label="워치리스트" value={report.watchlist.length} tone="neutral" />
              <DesktopStatCard label="생성 모델" value={report.modelName ?? '미상'} tone="neutral" />
            </div>
          ) : (
            <div className="space-y-4">
              <DesktopStatCard label="아카이브 수" value={archive.length} tone="neutral" />
            </div>
          )}
        />
                  ...
                  {report.sections.map((section) => (
                    <article key={section.id} className="border border-dot-border/60 bg-white/72 p-5">
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                          <h3 className="text-[18px] font-semibold tracking-[-0.02em] text-dot-accent">{section.title}</h3>
      ...
      <aside className="space-y-6">
        <DesktopSurface className="sticky top-7 p-5">
          <DesktopSectionHeader
            ...
            action={<span className="desktop-kicker">{archive.length} 회차</span>}
          />
            ...
            {archive.length > 0 ? (
              archive.map((item) => (
                <ArchiveCard
                  ...
                  hrefPrefix={archiveHrefPrefix}
                />
              ))
            ) : (
              <p className="text-[13px] leading-7 text-dot-sub">아카이브가 아직 준비되지 않았습니다.</p>
            )}
          </div>
        </DesktopSurface>
      </aside>
